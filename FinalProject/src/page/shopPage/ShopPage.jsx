...
import HeaderResponsive from "../../layout/headerResponsive/HeaderResp";
...
import notHaveProducts from "../../assets/image/mainPage/notFoundPage.png";
...
const ShopPage = () => {
  ...
  const filteredProducts = useSelector(
    (state) => state.filterSlider.filteredProducts
  );
  ...
  useEffect(() => {
    ...
    categoryProducts = categoryProducts.filter(
      (productItem) => productItem.price <= maxPrice
    );

    dispatch(
      updateFilteredProducts({ category: category, products: categoryProducts })
    );
  ...
  const productPriceLowToHigh = () => {
    ...
    dispatch(
      updateFilteredProducts({ category: category, products: sortedProducts })
    );
  };
  ...
  return (
    <div className={style.shopPage}>
      ...
      <div className={style.shopPage_header}>
        ...
        <div className={style.shopPage_header_right}>
          <div className={style.shopPage_header_right_title}>
            ...
            <IoIosArrowUp
              className={`${style.shopPage_header_right_title_icon} ${
                isOpen3
                  ? ""
                  : style.shopPage_header_right_title_icon_displayRotate
              }`}
      ...
      <div
        className={`${style.shopPage_header_right_card} ${
          isOpen3 ? "" : style.shopPage_header_right_card_displayNone
        }`}
      >
        ...
        <div className={style.shopPage_header_right_card_box}>
          <p
            className={style.shopPage_header_right_card_box_title}
            onClick={() => handleSelectedDropdown("Featured")}
          >
            Featured
          </p>
          <p
            className={style.shopPage_header_right_card_box_title}
            onClick={() => handleSelectedDropdown("Price high to low")}
          >
            Price high to low
          </p>
          <p
            className={style.shopPage_header_right_card_box_title}
            onClick={() => handleSelectedDropdown("Price low to high")}
          >
            Price low to high
          </p>
        </div>
      ...
      <div className="container">
        <p className={style.shopPage_goBack} onClick={() => navigate(-1)}>
          <IoIosArrowBack />
          Go Back
        </p>
                ...
                <div className={style.productCard} key={productItem.id}>
                  <div className={style.productCard_imgBox}>
                    ...
                    <img
                      ...
                      alt=""
                      className={style.shopPage_main_products_img}
                    />
                  </div>
                  <Link
                  ...
                  >
                    <p className={style.shopPage_main_products_title}>
                      {productItem.title}
                    </p>
                    <p className={style.shopPage_main_products_price}>
                      From {productItem.price}$
                    </p>
                  </Link>
                  <button
                    className={style.shopPage_main_products_btn}
                    onClick={() => handleAddToCart(productItem)}
                  >
                    Add To Cart
                    <span
                      className={style.shopPage_main_products_btn_span}
                    ></span>
                    <span
                      className={style.shopPage_main_products_btn_span}
                    ></span>
              ...
              <div className={style.notProductFoundCard}>
                <div className={style.notProductFoundCard_imgBox}>
                  <img
                    src={notHaveProducts}
                    alt=""
                    className={style.notProductFoundCard_imgBox_img}
                  />
                </div>
                <p className={style.notProductFoundCard_text}>
                  No Products Found
                </p>
              </div>
            )}
